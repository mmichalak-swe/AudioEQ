...
struct FFTDataGenerator
{
    ...
    {
        // when order is changed, recreate the window, forwardFFT, fifo, fftData
        // reset the fifoIndex
        // recreate anything that needs to be recreated, on the heap via std::make_unique<>
...
{
    /*
     converts 'renderData()' into a juceL::Path
     */
...
struct PathProducer
{
    PathProducer(SingleChannelSampleFifo<AudioEQAudioProcessor::BlockType>& scsf) :
...
private:
    SingleChannelSampleFifo<AudioEQAudioProcessor::BlockType>* leftChannelFifo;
